...
# feed url
repository_url="https://apoiston.github.io/openwrt-nikki"
if ! curl -s --head --fail --max-time 5 "$repository_url" > /dev/null; then
    repository_url="https://nikkifeeds.pages.dev"
fi
...
if [ -x "/bin/opkg" ]; then
    ...
    # add feed
    echo "add feed"
    if (grep -q -e nikki -e nikkifeeds /etc/apk/repositories.d/customfeeds.list); then
        sed -i '/nikki\|nikkifeeds/d' /etc/apk/repositories.d/customfeeds.list
    fi
